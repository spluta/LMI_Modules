BuchlaFilter_Mod {
	...
	*initClass {
		StartUp.add {
			SynthDef("buchlaLowFilter_mod", {arg inBus, freq, trigDB, dbFreq=1, dbRange=0, whichDB=0, dbNoiseOnOff = 0, lagTime=0, gate = 1, pauseGate = 1;
				var in, out, env, pauseEnv, db, dbNoise;
				...
				//dbNoise = LFNoise2.kr(dbFreq.poll);

				dbNoise = Ramp.kr(WhiteNoise.kr(1),1/dbFreq)*dbRange;

				db = Lag.kr(trigDB, lagTime)+(dbNoise*Lag.kr(dbNoiseOnOff, 0.1));

				//db.poll;

				db.clip(-12.0,12.0);

				out = BLowShelf.ar(in,freq,1, db);

				ReplaceOut.ar(inBus, out*env*pauseEnv);
			}).writeDefFile;
			SynthDef("buchlaMidFilter_mod", {arg inBus, freq, rq, trigDB, dbFreq=1, dbRange=0, dbNoiseOnOff = 0, whichDB=0, lagTime=0, gate = 1, pauseGate = 1;
				var in, out, env, pauseEnv, db, dbNoise;
			...
			SynthDef("buchlaHighFilter_mod", {arg inBus, freq, trigDB, dbFreq=1, dbRange=0, dbNoiseOnOff = 0, lagTime=0, gate = 1, pauseGate = 1;
				var in, out, env, pauseEnv, db, dbNoise;
	...
	trigger {arg lowDb, highDb, rqVal=1, lagTime = 0.05;
		...
		filter.set(\trigDB, randDB, \rq, qSet, \lagTime, lagTime);
		^[randDB, qSet]
	}

	setDBFreqRange {|dbFreq, dbRange|
		filter.set(\dbFreq, dbFreq, \dbRange, dbRange)
	}
...
BuchlaFiltersSynths_Mod {
	...
	*new {arg group, inBus, outBus;
	...
	setDBNoiseOnOff {|dbNoiseOnOff|
		filterList.do{|filter| filter.setDBNoiseOnOff(dbNoiseOnOff)};
	}
...
BuchlaFilters_Mod : Module_Mod {
	var buchlaFilters, trigVal, trigRout, dbFreq=1, dbRange=6;
	...
	init {
		...
		buchlaFilters = BuchlaFiltersSynths_Mod(group, mixerToSynthBus.index, outBus.index);

		controls.add(Button.new()
			.states_([ [ "Trigger", Color.red, Color.black ] ,[ "Trigger", Color.black, Color.red ] ])
			.action_{|v|
				if(buchlaFilters!=nil){buchlaFilters.trigger(trigVal)}
			}
		);
		controls.add(Button.new()
		...
		controls.add(QtEZSlider.new("trigVal", ControlSpec(1,12,'lin',1),
			{|v|
				...
				buchlaFilters.setDBFreqRange(dbFreq, dbRange);
		}, 6, orientation:\horz)
		);

		controls.add(Button.new()
			.states_([ [ "AutoTrigOff", Color.red, Color.black ] ,[ "AutoTrigOn", Color.black, Color.green ] ])
			.action_{|v|
				buchlaFilters.setDBNoiseOnOff(v.value);
		});
		this.addAssignButton(3,\onOff);

		controls.add(QtEZSlider.new("dbFreq", ControlSpec(0.01,2,'exp'),
			{|v|
				dbFreq = v.value;
				buchlaFilters.setDBFreqRange(dbFreq, dbRange);
		}, 1, orientation:\horz)
	...
	unpause {
		buchlaFilters.unpause
	}

	killMeSpecial {
		buchlaFilters.killMe
	}
}
